...
const props = defineProps({
    ...
    widows: { type: Object, required: true },
    sortOptions: { type: Array, default: () => [] },
});

const applyFilters = (nextFilters) => {
    router.get(route('manage.member-directory.widows.index'), pruneFilters(nextFilters), {
        preserveScroll: true,
        preserveState: true,
        replace: true,
    });
...
<template>
    <div class="space-y-6 p-6">
        <div class="space-y-4">
            <DirectorySectionTabs active="widows" />
            <div>
                <h1 class="text-3xl font-semibold text-surface-900 dark:text-surface-0">Widows</h1>
                <p class="mt-2 text-surface-600 dark:text-surface-300">
                    Surviving spouses connected to deceased lodge members through spouse relationships.
                </p>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-surface-200 bg-white p-4 shadow-sm dark:border-surface-800 dark:bg-surface-900">
            <div class="mb-4 text-sm text-surface-600 dark:text-surface-300">
                Showing {{ widows.from ?? 0 }}–{{ widows.to ?? 0 }} of {{ widows.total ?? 0 }} widows.
            </div>
            ...
            <DataTable
                ...
                paginator
                lazy
                :rows="widows.per_page"
                :first="((widows.current_page || 1) - 1) * (widows.per_page || 25)"
                :total-records="widows.total"
